...
- Add-on name: **Agent Connect**. Package/install folder: **`connect_plus`** (must be installed or symlinked into `addons21` under exactly this name — config resolution is keyed by folder name, see §6).
...
- Undo entry names (also returned as `undoEntry`):
  - `bulkAddNotes` → `"Agent Connect: Bulk Add"`
  - `bulkUpdateNoteFields` → `"Agent Connect: Bulk Update"`
  - `bulkAddTags` → `"Agent Connect: Bulk Tags"`
...
- **`undoLabel` (spec revision 9, see §24):** every Plus action that creates an undo entry accepts an optional `undoLabel` (default `null`). When given, `core.sanitize_undo_label` turns it into `"Agent Connect: " + <label>` (whitespace runs — newlines included — collapsed to single spaces, ends stripped, label capped at 80 chars), and that name replaces the action's default entry name everywhere the name is used: entry creation, merge target, atomic revert, and the response's `undoEntry` field. When `null`, the default names above are byte-for-byte unchanged. The `undoEntry` response field always reports the **actual** final entry name.
...
{"added": [1712345678901, ...], "suspended": [1712345678911, ...],
 "skipped": [{"index": 3, "reason": "duplicate"}], "undoEntry": "Agent Connect: Bulk Add"}
```
...
4. Write pass: for the first `ok` note, `target = col.add_custom_undo_entry("Agent Connect: Bulk Add")`. For each `ok` note: build Note, set fields/tags, `col.add_note(note, did)`, append `note.id` to `added`, `col.merge_undo_entries(target)`.
...
5. Hard error during the write pass (unexpected exception from Anki):
   - `atomic=true`: ensure entries are merged, then if `added` non-empty and `col.undo_status().undo == "Agent Connect: Bulk Add"`, call `col.undo()`. Then raise `[batch_reverted]` (§25) `"bulkAddNotes failed (batch reverted): " + json.dumps({"failedIndex": i, "error": str(e), "addedBeforeRevert": len(added), "skipped": skipped})`.
...
**Returns** `{"updated": [noteIds actually written], "unchanged": [noteIds whose requested values already matched], "skipped": [{"index", "reason"}], "suspensionPreserved": bool, "schedulingPreserved": bool, "undoEntry": "Agent Connect: Bulk Update" | null}` (`undoEntry` is `null` whenever `updated` is empty — nothing was written).
...
**Algorithm** — mirror of 4.1: validate → per-entry: load note, whole-entry field validation, no-op check (read-only, shared by the dry and real paths — see §15) → try: apply fields/tags, `col.update_note`, merge. Lazy undo entry `"Agent Connect: Bulk Update"`. Atomic revert + error report identical to 4.1 with prefix `"[batch_reverted] bulkUpdateNoteFields failed (batch reverted): "` (§25).
...
**Returns** `{"updated": [noteIds that actually changed], "skipped": [{"index", "reason"}], "undoEntry": "Agent Connect: Bulk Tags" | null}`
...
Single undo entry `"Agent Connect: Bulk Tags"` (lazy). Atomic contract identical, prefix `"[batch_reverted] bulkAddTags failed (batch reverted): "` (§25).
...
**Returns** `{"noteId": int, "cardIds": [int, ...], "undoEntry": str}` (cardIds ordered by card `ord`). `undoEntry` (added 2026-08-12, spec revision 9, §24): the actual top-of-stack undo entry name — the backend's own entry (locale-dependent, e.g. "Image Occlusion") by default, or `"Agent Connect: <label>"` when `undoLabel` is given (the custom entry then wraps the add AND the deck move; one undo reverts both, probe- and test-verified).
...
```json
{
  ...
  "errorPrefixNote": "Prefixing boundary: errors from the 37 Agent Connect actions AND the dispatcher's unknown-action error ...",
  "effectiveConfig": {
...
| 6 | `__init__.py:87` | `QMessageBox.critical(..., 'AnkiConnect', ...)` title → `'Agent Connect'`; message keeps the failing port number |
| 7 | `__init__.py:426-427` | permission-prompt text "…use Anki through AnkiConnect" → "…through Agent Connect" |

Optional (do it — personal fork, port-scoped clients):

| # | file:line | change |
|---|---|---|
| 8 | `web.py:185` | empty-body banner `{"apiVersion": "AnkiConnect v.6"}` → `{"apiVersion": "Agent Connect v.6"}`. Note in README: clients that sniff this exact string (Yomitan-style) must point at 8765/stock instead. |
...
1. **Name + one-liner**: Agent Connect — a personal fork of AnkiConnect adding bulk, image-occlusion, revlog, and backup actions on port **8766**.
...
4. **Coexistence note**: runs alongside stock AnkiConnect (2055492159) in the same Anki — stock on 8765, Plus on 8766; all upstream actions are also served on 8766; configs are independent; env overrides are `ANKICONNECT_PLUS_BIND_ADDRESS` / `ANKICONNECT_PLUS_CORS_ORIGIN`; banner string on 8766 reads "Agent Connect v.6".
...
Repo root `LICENSE` file = the **full, unmodified GNU GPLv3 text** (from https://www.gnu.org/licenses/gpl-3.0.txt). Keep upstream copyright notices in file headers; add a line `Copyright (C) 2026 Matthew Correll (Agent Connect modifications)` beneath the existing FooSoft notices in modified files, per GPLv3 §5(a).
...
| `noteIds` | [int] | omit | If given, every occurrence of the old filename in those notes' fields is rewritten to the new filename via `col.update_note`, all merged into ONE undo entry `"Agent Connect: Crop Image"` (lazy, per §3.3). Duplicate ids are deduplicated. |
...
```json
{"changed": 2, "changedIds": [1712345678901, 1712345678902], "undoEntry": "Agent Connect: Bulk Suspend"}
```
...
{"changed": 3, "changedIds": [1712345678901, 1712345678902, 1712345678903],
 ...
 "undoEntry": "Agent Connect: Bulk Due Date"}
```
...
{"changed": [1712345678901], "matchesTotal": 3, "unchanged": [1712345678902],
 ...
 "undoEntry": "Agent Connect: Replace in Fields"}
```
...
- `undoEntry`: `"Agent Connect: Replace in Fields"`, `null` when nothing was written. One merged entry; single `col.undo()` reverts the whole batch. Atomic failure raises `"bulkReplaceInFields failed (batch reverted): {json}"` with `{failedNoteId, error, changedBeforeRevert, skipped}` (noteId-keyed for the same reason as `skipped`).
...
- **Sanitization** (`core.sanitize_undo_label`, raised **before any write**): whitespace runs (newlines included) collapse to single spaces, ends stripped, label capped at **80 chars**; final name = `"Agent Connect: " + <label>`. Non-string → `"invalid parameter: undoLabel: string required"`; empty after sanitizing → `"invalid parameter: undoLabel: non-empty string required"`.
...
```json
{"undo": "Agent Connect: Bulk Update", "redo": null, "lastStep": 34}
```
...
{"renamed": [{"from": "HA2::PI 7", "to": "HA2::PI 07"}, {"from": "HA2::PI 7::Labs", "to": "HA2::PI 07::Labs"}],
 "configPreserved": true, "cardsAffected": 214, "undoEntry": "Agent Connect: Rename Deck"}
```
...
```json
{"updated": [1712345678901], "unchanged": [1712345678902], "undoEntry": "Agent Connect: Bulk Flag"}
```
...
**Edge cases tests must cover** — flag 3 cards (+1 bogus id, +1 duplicate) → `updated` names exactly the three, flags really `2`, ONE entry `"Agent Connect: Bulk Flag"`, single `col.undo()` restores the old flags; repeat → all `unchanged`, `undoEntry: null`, `undo_status()` bit-identical; partial pre-flagged split; `flag: 0` clears; `8` / `-1` / `true` → `[invalid_param]` with nothing written; dryRun split matches the following real run with zero writes; `undoLabel` honored.
...
{"notesUpdated": 3, "tagsRewritten": [{"from": "x::lab1", "to": "x::lab01"}, {"from": "x::lab1::sub", "to": "x::lab01::sub"}],
 "merged": [], "undoEntry": "Agent Connect: Rename Tag"}
```
...
**Edge cases tests must cover** — the prefix-safety lock: notes tagged `x::lab1`, `x::lab1::sub`, `x::lab10`, `X::LAB1` → rename `x::lab1` → `x::lab01`: dry `wouldRewrite` lists exactly the two pairs (NEVER `x::lab10`), real run reports `notesUpdated: 3` with `x::lab10` byte-untouched and the `X::LAB1` note rewritten (case-insensitive match), ONE entry `"Agent Connect: Rename Tag"`, single `col.undo()` restores the registry; merge onto a pre-existing `x::lab01` → `merged: ["x::lab01"]` on dry and real; ghost tag (register, delete note) → the no-write shape with `undo_status()` bit-identical; `[not_found]` for a missing tag; spacey `newTag` → `[invalid_param]` with the undo stack untouched; case-only rename lands; byte-identical no-op; `undoLabel` honored.
...
Two new actions + one deliberate behavior change (§17, §0 Deviation #14). Both actions follow §3.3 undo conventions where they write, take §24's `undoLabel` and §15's `dryRun` where they write, and raise §25-coded errors. New entry name: `"Agent Connect: Empty Filtered Deck"`. No new config keys. Card-location selects (`did`/`odid`) are read-only and explicitly allowed by the §0 HARD-RULES bullet, which names this section. **Revision 19 (§32) adds the build half of the same lifecycle — `createFilteredDeck` / `rebuildFilteredDeck`; §29's two actions are its census (`filteredDeckReport`) and remediation (`emptyFilteredDeck`) halves.**
...
```json
{"returned": 5, "homeDecks": {"HA2::PI 7": 3, "HA2::PI 8": 2}, "undoEntry": "Agent Connect: Empty Filtered Deck"}
```
...
{"cardsDeleted": 2, "deletedCardIds": [1712345678955, 1712345678957], "notesAffected": 2,
 ...
 "skipped": [], "undoEntry": "Agent Connect: Delete Empty Cards"}
```
...
{"deckId": 1723456789012, "name": "PI9 cram", "cardsGathered": 52,
 ...
 "undoEntry": "Agent Connect: Create Filtered Deck"}
```
...
```json
{"cardsGathered": 48, "returnedFirst": 52, "undoEntry": "Agent Connect: Rebuild Filtered Deck"}
```
...
| `undoLabel` | str | `null` | §24; names the tag write's undo entry. Default entry: `"Agent Connect: Stage Optional Tag"`. |
...
{"tagged": [1483845152253], "alreadyTagged": [1489115655836],
 ...
 "undoEntry": "Agent Connect: Stage Optional Tag"}
```
