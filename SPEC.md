...
### 4.9 `plusInfo` (amended 2026-08-12, spec revision 7: `actionDocs`; revision 10: `recipes`; revision 13: `returns`, `errorCodes`, `errorPrefixNote`)
...
```json
{
  ...
  "errorPrefixNote": "Prefixing boundary: errors from the 34 Plus actions AND the dispatcher's unknown-action error ...",
  "docs": {
    "plus": "<DOCS_PLUS>",
...
**Edge cases** — callable before profile load (signature reflection touches no collection); callable through `multi`; **callable during a sync** (`guard_sync=False`, §25.2); action list exactly matches `core.PLUS_ACTIONS` (single source of truth — test asserts every listed name is a dispatchable `@util.api()` method); every `actionDocs` entry has exactly `{summary, params, returns}`, a non-empty `summary`, a `params` string matching the wrapper's real signature, and a non-empty `returns`; `errorCodes` covers the vocabulary exactly (incl. `unknown_action`) with `retryable` matching `PLUS_ERROR_CODES` per code and every reserved code's `meaning` saying so.
...
```json
{"path": "/Users/mattyc/Downloads/HA2-PI-7-2026-08-11.apkg", "sizeBytes": 152344, "notesExported": 214, "warnings": []}
```
...
Cross-cutting contract on **every Plus action** (26 at revision 10, **27** since revision 12's §26 `undoStatus`, **30** since revision 17's §28 maintenance actions, **34** since revision 17 slice 2's §§29–30 filtered-deck/empty-cards actions; amends §3.2; every action's error list now shows its codes). Rationale (round-2 field feedback, measured in real use): callers pattern-matched free-text error messages to decide whether to retry, fix params, or give up — brittle and English-locked.
...
| `duplicate` | no | **yes (new in revision 17)** | `renameDeck`'s occupied-name refusal: `"deck already exists: <name>"` when `newName` (or an implied descendant name) resolves to a deck OUTSIDE the renamed subtree (§28.1) — the backend would silently auto-rename instead. NOTE-level duplicates are still per-item `skipped[].reason` strings, never raised |
...
| `validation_error` | no | yes | well-formed request refused on semantic grounds: not-an-IO-note, IO note without an image, non-rect/unpreservable/mixed-oi occlusions, crop-drops-all-occlusions, AnkiHub HTTP 400 (`VALIDATION_ERROR:` taxonomy) and note-already-on-AnkiHub |
...
| `bulkAddNotes` (§4.1) | `suspend` | `suspendNewCards` | `true` | The batch's new cards are left suspended and reported in `suspended`. |
...
- Byte-identical `newName` → `{renamed: [], configPreserved: true, cardsAffected: 0, undoEntry: null}`, nothing written, undo stack untouched.
- `dryRun: true` → `{wouldRename: [{from, to}], cardsAffected, undoEntry: null}` — `wouldRename` is a **prediction** from the pre-op names (suffix arithmetic against the stored root spelling), which is why it is renamed from `renamed` (an observation). The `[duplicate]`/`[deck_not_found]`/`[invalid_param]` refusals fire on the dry path too, so the prediction is exactly what the real call would land (fix pass: un-normalized `newName` is refused rather than predicted un-normalized).
...
Two new actions + one deliberate behavior change (§17, §0 Deviation #14). Both actions follow §3.3 undo conventions where they write, take §24's `undoLabel` and §15's `dryRun` where they write, and raise §25-coded errors. New entry name: `"AnkiConnect Plus: Empty Filtered Deck"`. No new config keys. Card-location selects (`did`/`odid`) are read-only and explicitly allowed by the §0 HARD-RULES bullet, which names this section.
